...
export function layoutLinks(centerId, links, width, height) {
  ...
  const counts = { archive: links.filter((link) => link.kind === 'archive').length, class: links.filter((link) => link.kind === 'class').length };
  const sequence = { archive: 0, class: 0 };
  const nodes = [{ id: centerId, kind: 'center', x: center.x, y: center.y, fx: center.x, fy: center.y }];
  for (const link of links) {
    const radius = link.kind === 'archive' ? inner : outer;
    const angle = -Math.PI / 2 + (sequence[link.kind]++ / counts[link.kind]) * Math.PI * 2 + (link.kind === 'archive' ? 0.36 : 0);
