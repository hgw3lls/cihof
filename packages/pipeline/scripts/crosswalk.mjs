...
import { crosswalkProgress, unresolvedEntries } from '@cihof/content';
import { buildInductionCrosswalk } from '../src/build/crosswalk.ts';
import { dataFile } from '../src/paths.ts';

/**
 * Refreshes data/cihof_induction_crosswalk.json.
 *
 * Safe to re-run. Existing resolutions are carried forward by the recorded
 * name; only the roster-derived parts are rebuilt. Pass --check to verify the
 * file is current without writing, which is what CI runs.
 */
...
if (check) {
  ...
  console.log(`Induction crosswalk is current: ${progress.total} names, ${progress.unresolved} unresolved.`);
  process.exit(0);
}

if (!current) writeFileSync(target, `${JSON.stringify(crosswalk, null, 2)}\n`);
